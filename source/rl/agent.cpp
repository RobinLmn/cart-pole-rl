...
agent::agent(const neural_network& policy, const float gamma, const float learning_rate)
...
int agent::act(const std::vector<float>& state) const
{
...
void agent::learn()
{
    ...
    for (int t = 0; t < transition_size; ++t)
    {
        ...
        const int action = transition.action;
